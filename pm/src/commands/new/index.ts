...
async function loadTemplate(cwd: string, type: "story" | "task"): Promise<string> {
	...
	const bundledPath = fileURLToPath(
		new URL(`../../../template/templates/${type}.md`, import.meta.url),
	);
...
export async function run(rawOpts: Record<string, unknown>, args?: string[]): Promise<void> {
	...
	if (!message) {
		throw new PmError(
			'Error: message argument is required.\n  Usage: pm new "title @assignee #layer key:value"',
			1,
		);
	}
	...
	if (parsed.assignee === "me") {
		...
		if (!user) {
			throw new PmError(
				"Error: could not resolve current user from GitHub remote or git config.",
				1,
			);
		}
