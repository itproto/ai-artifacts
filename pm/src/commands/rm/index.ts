...
export async function run(rawOpts: Record<string, unknown>, args?: string[]): Promise<void> {
	...
	if (opts.dryRun) {
		console.log(`dry-run  would close ${selected.id}  ${selected.title}  (${reason || 'no reason'})`)
		return
	}
...
async function pickFromList(items: BoardItem[], prompt: string): Promise<BoardItem> {
	...
	for (let i = 0; i < items.length; i++) {
		...
		console.log(`  ${String(i + 1).padStart(2)}  ${item.id.padEnd(10)}  ${item.title.slice(0, 50)}${assignee}`)
	}
	console.log()
